...
const Companies = () => {
    ...
    const [selectedCompany, setSelectedCompany] = useState(null); // For Detail Popup
    const itemsPerPage = 8;

    const filtered = companies.filter(c => {
        const matchesSearch = c.name.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesIndustry = industryFilter === "" || c.industry === industryFilter;
        const matchesLocation = locationFilter === "" || c.location.includes(locationFilter);
    ...
    return (
        <div className="relative">
            ...
            <div className="flex justify-between items-center mb-8 px-2">
                <div className="text-slate-400 text-[11px] font-black uppercase tracking-widest leading-none">
                    {/* <span className="text-slate-900 text-2xl mr-1 tracking-tighter">{filtered.length}</span> Results Found */}
                </div>
                <div className="flex bg-[#F1F5F9] p-1.5 rounded-xl shadow-inner border border-slate-100">
                    <button onClick={() => setView('grid')} className={`flex items-center gap-1.5 px-5 py-2 rounded-xl text-[10px] font-black transition-all ${view === 'grid' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'text-slate-500'}`}>
                        <LayoutGrid size={13} /> Grid
                    </button>
                    <button onClick={() => setView('table')} className={`flex items-center gap-1.5 px-5 py-2 rounded-xl text-[10px] font-black transition-all ${view === 'table' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'text-slate-500'}`}>
                        <List size={13} /> Table
                    </button>
                </div>
            </div>
            ...
            {view === 'table' ? (
                /* --- TABLE VIEW --- */
                <div className="bg-white rounded-[2rem] border border-slate-200/60 shadow-sm overflow-hidden mx-2">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {currentItems.map((c) => (
                                ...
                                >                                   
                                 <td className="px-8 py-3 flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center p-2 shadow-sm overflow-hidden flex-shrink-0">
                                            <img src={c.logo} className="w-full h-full object-contain mix-blend-multiply" alt="" />
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-1.5"><span className="font-bold text-slate-800 text-[13px] tracking-tight">{c.name}</span><CheckCircle2 size={13} className="text-blue-500 fill-blue-50" /></div>
                                            <span className="text-[10px] text-slate-400 font-bold lowercase tracking-tight">{c.website}</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-3 text-[11px] font-black text-slate-600 uppercase tracking-widest">{c.industry}</td>
                                    <td className="px-8 py-3 text-[12px] text-slate-500 font-medium">{c.location}</td>
                                    <td className="px-8 py-3 text-[12px] text-slate-500 font-black">{c.size}</td>
                                    <td className="px-8 py-3">
                                        <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${c.status === 'Active' ? 'bg-[#E6F9F0] text-[#00C48C]' : 'bg-[#FFF8E6] text-[#FFA800]'}`}>{c.status || 'Active'}</span>
                                    </td>
                                    <td className="px-8 py-3 text-right">
                                        <button onClick={() => setSelectedCompany(c)} className="text-blue-600 font-black text-[10px] uppercase tracking-widest flex items-center gap-1 ml-auto group-hover:translate-x-1 transition-transform">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-2">
                    {currentItems.map(company => (
                        <div key={company.id} className="bg-white p-6 rounded-[1.5rem] border border-slate-100 shadow-sm relative hover:shadow-xl hover:border-blue-100 transition-all group flex flex-col h-full">
            ...
            {/* --- BRANDED SMALL PAGINATION (TEXT LEFT, BUTTONS RIGHT) --- */}
            <div className="mt-12 flex flex-row justify-between items-center px-4">
                ...
                <div className="flex items-center gap-1.5">
                    <button
                        ...
                        className="w-8 h-8 flex items-center justify-center bg-white rounded-lg border border-slate-100 text-[#94A3B8] disabled:opacity-20 hover:text-blue-600 hover:bg-slate-50 transition-all shadow-sm"
                    >
                    ...
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="relative p-8">
                            ...
                            <div className="flex items-center gap-5 mb-8">
                                <div className="w-20 h-20 bg-white border border-slate-100 rounded-3xl p-3 shadow-sm">
                                    <img src={selectedCompany.logo} className="w-full h-full object-contain" alt="" />
                                </div>
                                <div>
                                    <div className="flex items-center gap-2">
                                        <h2 className="text-2xl font-black tracking-tight text-slate-900">{selectedCompany.name}</h2>
                                        <CheckCircle2 size={20} className="text-blue-500 fill-blue-50" />
                                    </div>
                                    <p className="text-blue-600 font-bold text-sm uppercase tracking-widest">{selectedCompany.industry}</p>
                                </div>
                            </div>
